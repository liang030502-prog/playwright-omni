...
tasks = [
    ...
    ('赛题6:Health Science', 'https://duckduckgo.com/?q=Nestl%C3%A9+Health+Science+nutrition+strategy+2025+2026', '找到Nestl%C3%A9+Health+Science营养改善相关信息'),
...
for name, url, goal in tasks:
    ...
    for s in result.steps:
        if s.success and s.thought:
            print(f'  Step{s.step}: {s.thought[:300]}')
    if result.error_message:
        print(f'Error: {result.error_message}')
    results.append((name, result))
print('\n\n=== FINAL SUMMARY ===')
for name, r in results:
    print(f'{name}: status={r.status}, steps={r.total_steps}')
    if r.success:
        print(f'  Final thought: {r.steps[-1].thought[:500] if r.steps else "no steps"}')
    print()
